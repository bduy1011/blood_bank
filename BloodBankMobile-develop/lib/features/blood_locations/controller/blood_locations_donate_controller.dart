...
class BloodLocationsController extends BaseModelStateful {
  ...
  Future<void> cancelRegisterDonate(
    ...
    try {
      ...
      hideLoading();
      if (response.status == 200) {
        AppUtils.instance.showToast(AppLocale.cancelRegisterSuccess.translate(Get.context!));
        loadData();
      }
    ...
    try {
      ...
      if (response.status == 200) {
        if (response.data?.firstOrNull?.googleMapLink?.isNotEmpty == true) {
          ...
          AppUtils.instance.showToast(AppLocale.cannotFindRoute.translate(Get.context!));
        }
      }
